...
export class News extends React.Component {
  ...
  componentDidMount() {
    const code =
      navigator.language.split('-').length == 1 ? navigator.language.split('-')[0] : navigator.language.split('-')[1];

    axios.get(`https://api.dothq.co/browser.news?country=${code.toLowerCase()}`).then((resp) => {
