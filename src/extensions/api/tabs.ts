...
export class Tabs {
  ...
  public query = (queryInfo: chrome.tabs.QueryInfo, callback: (tabs: chrome.tabs.Tab[]) => void) => {
    ipcRenderer.send('api-tabs-query');

    ipcRenderer.once('api-tabs-query', (e: Electron.IpcMessageEvent, data: chrome.tabs.Tab[]) => {
  ...
  public create = (createProperties: chrome.tabs.CreateProperties, callback: (tab: chrome.tabs.Tab) => void = null) => {
    ipcRenderer.send('api-tabs-create', createProperties);

    if (callback) {
      ipcRenderer.once('api-tabs-create', (e: Electron.IpcMessageEvent, data: chrome.tabs.Tab) => {
        callback(data);
      });
    }
  };
  ...
  public insertCSS = (arg1: any = null, arg2: any = null, arg3: any = null) => {
    const insertCSS = (tabId: number, details: any, callback: any) => {
      if (details.hasOwnProperty('file')) {
        details.code = readFileSync(join(api._extension.path, details.file), 'utf8');
      }
  ...
  public executeScript = (arg1: any = null, arg2: any = null, arg3: any = null) => {
    const executeScript = (tabId: number, details: any, callback: any) => {
      ...
      ipcRenderer.once('api-tabs-executeScript', (e: Electron.IpcMessageEvent, result: any) => {
        if (callback) {
          callback(result);
        }
      });
  ...
  public getZoom = (tabId: number, callback: (zoomFactor: number) => void) => {
    ...
    ipcRenderer.once('api-tabs-getZoom', (e: Electron.IpcMessageEvent, zoomFactor: number) => {
      if (callback) {
        callback(zoomFactor);
      }
    });
  ...
  public detectLanguage = (tabId: number, callback: (language: string) => void) => {
    ...
    ipcRenderer.once('api-tabs-detectLanguage', (e: Electron.IpcMessageEvent, language: string) => {
      if (callback) {
        callback(language);
      }
    });
